...
import com.allcam.utils.SpringContextHolder;
...
public class SystemLogAspect
{
    // 注入Service用于把日志保存数据库
    @Resource
    private AllcamLogService allcamLogService;
    ...
    // Controller层切点
    @Pointcut("@annotation(com.allcam.log.SystemControllerLog)")
    ...
    public void doBefore(JoinPoint joinPoint)
    {
        ...
        // 读取session中的用户
        AuthLoginResUserInfo user = (AuthLoginResUserInfo)session.getAttribute(ServiceContants.CURRENT_USER);
        // 请求的IP
        ...
        try
        {
            ...
            log.setCreateDate(DateUtil.getCurrentDate(DateUtil.DATE_19));
            // 保存数据库
            allcamLogService.save(log);
